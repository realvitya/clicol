...
def init(ct):
    ...
    cmap=[
        ...
        [4,"prompt","",re.compile(r"(?i)^((?:[\b]+\ *[\b]*)?[a-z0-9\/_\-]+@[a-z0-9\/_\-]*(?:>|\$) )(.*)$"), "".join((ct['privprompt'],r"\1",ct['default'],r"\2")),BREAK],

        # interface names
        [15,"","",re.compile(BOS+r"\b(?<![\(\)\[\]\\\/.-:])((?:(?:(?:[gx]e|gr|ip|[lmuv]t|p[de]|pf[eh]|lc|lsq|sp)-|ae|em|fxp|lo|lis|me|pip|pp)[0-9]{1,4}|(?:reth|irb|cbp|lsi|mtun|pim[de]|tap|dsc|demux)[0-9]?)(?:[\/.][0-9]{1,5})*)\b(?! -)"), "".join((ct['interface'],r"\1",ct['default'])),CONT],

        #BGP
        # Juniper show bgp sum
        #Groups: 8 Peers: 17 Down peers: 0
        [20,"","",re.compile(BOL+r"(Groups: [0-9]+ Peers: [0-9]+ Down peers: )(0)"), "".join((r"\1\2",ct['good'],r"\3",ct['default'])),CLEAR],
        [20,"","",re.compile(BOL+r"(Groups: [0-9]+ Peers: [0-9]+ Down peers: )([0-9]+)"), "".join((r"\1\2",ct['alert'],r"\3",ct['default'])),CLEAR],
        # Established
