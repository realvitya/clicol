...
def init(ct):
  ...
  cmap=[
      ...
      #IPv4 address
      ["","",re.compile(BOS+r"((?:[0-9]{1,3}\.){3}[0-9]{1,3}(?:\/[0-9]{1,2})?)",flags=re.M), ct['address']+r"\1"+ct['default'],CONT],
      #IPv6 address
      ...
      # admin down
      ["","",re.compile(BOS+r"(admin(?:istratively)? down)",flags=re.M),ct['alert']+r"\1"+ct['default'],CONT],
      ["","",re.compile(BOS+r"(disabled|[\ \(,][dD]own[\ \(,]?)",flags=re.M),ct['alert']+r"\1"+ct['default'],CONT],
      ["","",re.compile(BOS+r"(enabled|[\ \(,][uU]p[\),\ ]?)",flags=re.M),ct['good']+r"\1"+ct['default'],CONT],
